...
public class DeathDrops
{
	...
	public void onDeath(LivingDeathEvent e)
	{
		if(e.entityLiving instanceof EntityPig && e.source.getEntity() instanceof EntityPlayer)
		{
			...
			if(player.getCurrentEquippedItem() == null ||(player.getCurrentEquippedItem() != null && player.getCurrentEquippedItem().getItem() != BaconItems.fineSword))
			{
				if(rand.nextBoolean() == true)
				{
					if(BaconItems.rawBacon != null)
					{
						pig.dropItem(BaconItems.rawBacon, 1);
						return;
					}
				}
			}
			...
			if(rand.nextInt(3) == 0)
			{
				...
				{
					pig.dropItem(BaconItems.rottedBacon, 1);
					return;
				}
			}
			else if(rand.nextInt(3) == 1)
			{
				...
				{
					pig.dropItem(BaconItems.rottedPorkChop, 1);
					return;
				}
