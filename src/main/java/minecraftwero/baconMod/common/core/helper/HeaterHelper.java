...
public class HeaterHelper
{
	...
	{
		BaconItems.liquidBacon.setIcons(evt.map.registerIcon(BaconLib.modID
				.toLowerCase() + ":liquidBacon"));
	}
	...
	{
		if (BiomeDictionary.isBiomeOfType(par1, Type.BEACH)
				|| BiomeDictionary.isBiomeOfType(par1, Type.SWAMP)
				|| BiomeDictionary.isBiomeOfType(par1, Type.WATER))
		{
			return 20;
		...
		{
			return -10;
		} else if (BiomeDictionary.isBiomeOfType(par1, Type.MOUNTAIN)
				|| BiomeDictionary.isBiomeOfType(par1, Type.HILLS))
		{
			return 5;
