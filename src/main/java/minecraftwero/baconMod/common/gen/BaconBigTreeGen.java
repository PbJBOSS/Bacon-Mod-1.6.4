...
public class BaconBigTreeGen extends WorldGenerator
{
    /**
    ...
    public BaconBigTreeGen(boolean par1)
    {
    	this(par1, BaconItems.baconMachine.blockID, 4, BaconItems.baconMachine.blockID, 5);
    }
    
    public BaconBigTreeGen(boolean par1, int logBlockID, int logBlockMeta, int leaveBlockID, int leaveBlockMeta)
    {
    	LogID = logBlockID;
        ...
        for (aint1[par5] = aint[par5]; j1 <= i1; ++j1)
        {
            ...
            while (k1 <= i1)
            {
                ...
                else
                {
                    ...
                    {
                        this.setBlockAndMetadata(this.worldObj, aint1[0], aint1[1], aint1[2], LeaveID, LeaveMeta);
                        ++k1;
                    }
                }
            }
    ...
    void generateLeafNode(int par1, int par2, int par3)
    {
        ...
        for (int i1 = par2 + this.leafDistanceLimit; l < i1; ++l)
        {
            ...
            this.genTreeLayer(par1, l, par3, f, (byte)1);
        }
    }
    ...
    void placeBlockLine(int[] par1ArrayOfInteger, int[] par2ArrayOfInteger)
    {
        ...
        if (aint2[b1] != 0)
        {
            ...
            for (int k = aint2[b1] + b4; j != k; j += b4)
            {
                ...
                this.setBlockAndMetadata(this.worldObj, aint3[0], aint3[1], aint3[2], LogID, LogMeta);
            }
        }
    }
    ...
    int checkBlockLine(int[] par1ArrayOfInteger, int[] par2ArrayOfInteger)
    {
        ...
        else
        {
            ...
            for (j = aint2[b1] + b4; i != j; i += b4)
            {
                ...
                int t = this.worldObj.getBlockMetadata(aint3[0], aint3[1], aint3[2]);
                if (k != 0 && k != LeaveID && t != LeaveMeta)
                {
                    break;
                }
            }
    ...
    boolean validTreeLocation()
    {
        ...
        boolean isValidSoil = (soil != null && soil.canSustainPlant(worldObj, basePos[0], basePos[1] - 1, basePos[2], ForgeDirection.UP, (BlockBaconSapling)BaconItems.baconSapling));
        if (!isValidSoil)
        {
            return false;
        }
